...
Base and credit by: DARK N HACKER 
"""
...
DEFAULT_ADMIN_IDS = ['7306009706']

def main():
    """Main function to start the bot"""
    
    # Get configuration from environment; keep the bot token out of source control.
    token = os.getenv('TELEGRAM_BOT_TOKEN')
    configured_admin_ids = [item.strip() for item in os.getenv('ADMIN_IDS', '').split(',') if item.strip()]
    admin_ids = list(dict.fromkeys(configured_admin_ids + DEFAULT_ADMIN_IDS))
    ...
    if not admin_ids:
        print("⚠️ Warning: No admin IDs configured. Some commands will be restricted.")
